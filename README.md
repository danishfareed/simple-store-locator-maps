...
- App URL: `https://simple-store-locator.fareedanish1.workers.dev` (set in [`shopify.app.toml`](shopify.app.toml) and [`wrangler.toml`](wrangler.toml))
...
This runs `npm run build` (React Router client/server build → `node scripts/build-worker.mjs` → `node scripts/build-extension.mjs`, emitting the Worker bundle, the storefront widget bundle, and the admin preview bundle) followed by `wrangler deploy`. Once it completes, the app is live at `https://simple-store-locator.fareedanish1.workers.dev`.
